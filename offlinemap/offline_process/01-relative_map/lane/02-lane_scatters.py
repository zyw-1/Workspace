"""
将lane根据曲率，离散成固定距离的散点，生成新表scatter_in_lane
"""
...
def linestring_split(lane_id,utm,rs):
    accu_length = 0
    ...
    while accu_length < total_length or len(l) == 0:
        ...
        # TODO: 生成第一个点，最后一个点到实际线的终点距离为0.5m
        pnt = utm.interpolate(accu_length)
        sequence += 1
        # interval表示该点到下一个点的实际距离
        l.append({'lane_id': lane_id, 'sequence': sequence, 'geom': ctf.lonlat(pnt).wkt, 'utm':pnt.wkt,
    ...
    l[-2]['interval'] = l[-2]['interval'] - 0.5
    end_pnt = utm.interpolate(total_length-0.5)
    l[-1]['utm'] = end_pnt.wkt
    ...
    for index,row in df_lane.iterrows():
        ...
        for i in kappas:
            if i == 0:
                ...
                rs.append(1/i)
        l += linestring_split(lane_id,utm,rs)
    df=pd.DataFrame(l)

    sql = "drop table if exists scatter_in_lane;"
    pg_map.execute(sql)
    pg_map.df_to_pg(df, 'scatter_in_lane')
    sql = ("alter table scatter_in_lane alter column geom type geometry;"
           "alter table scatter_in_lane alter column utm type geometry;")
    pg_map.execute(sql)
