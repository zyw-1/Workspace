# -*- coding: UTF-8 -*-
"""
将所有元素都添加到一张大表中，生成新表pm_all_features
"""
import os
import sys
path = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
sys.path.append(path)
from lib.config import pg_map as pg,ctf,srid
import pandas as pd
...
dic_type = {'pm_cross_walks':'cross_walk','pm_junction_lights':'junction_light','pm_lane_arrows':'lane_arrow',
             'pm_stop_lines':'stop_line','pm_lane_lines':'lane_line','pm_feature_points':'feature_point',}
dic_id = {'pm_cross_walks':'cross_walk_id','pm_junction_lights':'junction_light_id','pm_lane_arrows':'lane_arrow_id',
          'pm_stop_lines':'stop_line_id','pm_lane_lines':'line_id','pm_feature_points':'feature_point_id'}
...
for table in dic_type:
    print(table)
    df= pg.get(table)
    if table not in ['pm_stop_lines','pm_junction_lights','pm_cross_walks']:
        for index,row in df.iterrows():
            ...
            if table == 'pm_lane_arrows':
                print(int(alane_id))
            link_id = row['link_id']
            dic = {'id':id,'type':type,'geometry':geometry,'utm':utm,'dist_on_alane':float(dist),'alane_id':alane_id,'link_id':link_id}
            l.append(dic)
    else:
        for index,row in df.iterrows():
            id = row[dic_id[table]]
            ...
            utm = ctf.utm(wkt.loads(geometry)).wkt
            dist = round(row['dist_on_alane'],2)
            alane_ids = row['alane_ids']
            link_id = row['link_id']
            for alane_id in alane_ids.split(':'):
                dic = {'id':id,'type':type,'geometry':geometry,'utm':utm,'dist_on_alane':float(dist),'alane_id':alane_id,'link_id':link_id}
                l.append(dic)
...
sql = ("alter table pm_all_features alter column alane_id type int USING alane_id::integer;")
pg.execute(sql)
